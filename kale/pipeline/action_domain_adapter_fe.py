...
class DANNtrainer4Video(DANNtrainer):
        ...
        if self.feat is not None:
            if self.image_modality in ['rgb', 'flow']:
                if self.rgb_feat is not None:
                    ...
                    at4b, at4c, at4d, at4e, at4f = self.rgb_feat(x)
                else:
                    x = self.flow_feat(x)
                x = x.view(x.size(0), -1)
                class_output = self.classifier(x)
    ...
    def compute_loss(self, batch, split_name="V"):
        ...
        #     }
            ...
            (x_s, y_s), (x_tu, y_tu) = batch
            _, y_hat, d_hat, \
            d_c4b_hat, d_c4c_hat, d_c4d_hat, d_c4e_hat, d_c4f_hat, \
            d_t4b_hat, d_t4c_hat, d_t4d_hat, d_t4e_hat, d_t4f_hat, \
            c4b, c4c, c4d, c4e, c4f, \
            t4b, t4c, t4d, t4e, t4f, \
            ac4b, ac4c, ac4d, ac4e, ac4f, \
            at4b, at4c, at4d, at4e, at4f = self.forward(x_s)

            t_x, y_t_hat, d_t_hat, \
            d_t_c4b_hat, d_t_c4c_hat, d_t_c4d_hat, d_t_c4e_hat, d_t_c4f_hat, \
            d_t_t4b_hat, d_t_t4c_hat, d_t_t4d_hat, d_t_t4e_hat, d_t_t4f_hat, \
            t_c4b, t_c4c, t_c4d, t_c4e, t_c4f, \
            t_t4b, t_t4c, t4d, t_t4e, t_t4f, \
            t_ac4b, t_ac4c, t_ac4d, t_ac4e, t_ac4f, \
            t_at4b, t_at4c, t_at4d, t_at4e, t_at4f = self.forward(x_tu)
 ...
 \
    ...
    def training_epoch_end(self, outputs):
        if self.current_epoch == 1:
            ...
            self.logger.experiment.add_graph(DANNtrainer4Video(
                self._dataset, self.image_modality, self.feat, self.classifier, self.domain_classifier, self.method),
                sample)
    ...
    def test_step(self, batch, batch_nb):
        ...
        # save("t_c4f.npy", t_c4f)
        # save("t_ac4f.npy", t_ac4f)
