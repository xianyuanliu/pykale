...
# Construct a dataset for videos with data with multiple modalities."""
...
class VideoMultiModalDatasets(MultiDomainDatasets):
    ...
    def prepare_data_loaders(self):
        if self.rgb:
            logging.debug("Load RGB train and valid")
            (self._rgb_by_split["train"], self._rgb_by_split["valid"]) = self._access_dict["rgb"].get_train_valid(
        ...
        if self.flow:
            logging.debug("Load flow train and valid")
            (self._flow_by_split["train"], self._flow_by_split["valid"]) = self._access_dict["flow"].get_train_valid(
                self._valid_split_ratio
            )
        ...
        if self.audio:
            logging.debug("Load audio train and val")
            (self._audio_by_split["train"], self._audio_by_split["valid"]) = self._access_dict["audio"].get_train_valid(
                self._valid_split_ratio
            )
