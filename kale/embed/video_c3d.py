...
"""
Define Convolutional 3D ConvNets(C3D) on Action Recognition.
...
class C3D(nn.Module):
    ...
    def __init__(self, num_classes=8, input_channel=3, pretrained=False):
        super(C3D, self).__init__()

        self.conv1 = nn.Conv3d(input_channel, 64, kernel_size=(3, 3, 3), padding=(1, 1, 1))
    ...
    def forward(self, x):
        x = self.relu(self.conv1(x))  # batch_size, 3, 16, 112, 112
        x = self.pool1(x)

        x = self.relu(self.conv2(x))  # batch_size, 64, 16, 56, 56
        x = self.pool2(x)

        x = self.relu(self.conv3a(x))  # batch_size, 128, 8, 28, 28
        x = self.relu(self.conv3b(x))
        x = self.pool3(x)

        x = self.relu(self.conv4a(x))  # batch_size, 256, 4, 14, 14
        x = self.relu(self.conv4b(x))
        x = self.pool4(x)

        x = self.relu(self.conv5a(x))  # batch_size, 512, 2, 7, 7
        x = self.relu(self.conv5b(x))
        x = self.pool5(x)

        x = x.view(-1, 8192)  # batch_size, 512, 1, 4, 4
        x = self.relu(self.fc6(x))  # batch_size, 8192
        x = self.dropout(x)
        x = self.relu(self.fc7(x))  # batch_size, 4096
        x = self.dropout(x)
    ...
    def __load_pretrained_weights(self):
        ...
        p_dict = torch.load(Path(__file__).absolute().joinpath(model_urls["c3d_pretrained"]))
