...
class InceptionModule(nn.Module):
    ...
    def forward(self, x):
        ...
        if "SELayerC" in dir(self):  # Check self.SELayer
            out, out_c, a_c = self.SELayerC(out)
        if "SELayerT" in dir(self):
            out, out_t, a_t = self.SELayerT(out)
        ...
        if "SELayerCTc" in dir(self):
            out, out_c, a_c = self.SELayerCTc(out)
        if "SELayerCTt" in dir(self):
            out, out_t, a_t = self.SELayerCTt(out)

        if "SELayerTCt" in dir(self):
            out, out_t, a_t = self.SELayerTCt(out)
        if "SELayerTCc" in dir(self):
            out, out_c, a_c = self.SELayerTCc(out)

        return out, out_c, out_t, a_c, a_t
...
class InceptionI3d(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self._modules["Conv3d_2c_3x3"](x)  # [2, 192, 1, 56, 56]
        x = self._modules["MaxPool3d_3a_3x3"](x)  # [2, 192, 1, 28, 28]
        x, _, _, _, _ = self._modules["Mixed_3b"](x)  # [2, 256, 1, 28, 28]
        x, _, _, _, _ = self._modules["Mixed_3c"](x)  # [2, 480, 1, 28, 28]
        x = self._modules["MaxPool3d_4a_3x3"](x)  # [2, 480, 1, 14, 14]
        x_4b, c4b, t4b, ac4b, at4b = self._modules["Mixed_4b"](x)  # [2, 512, 1, 14, 14]
        x_4c, c4c, t4c, ac4c, at4c = self._modules["Mixed_4c"](x_4b)  # [2, 512, 1, 14, 14]
        x_4d, c4d, t4d, ac4d, at4d = self._modules["Mixed_4d"](x_4c)  # [2, 512, 1, 14, 14]
        x_4e, c4e, t4e, ac4e, at4e = self._modules["Mixed_4e"](x_4d)  # [2, 528, 1, 14, 14]
        x_4f, c4f, t4f, ac4f, at4f = self._modules["Mixed_4f"](x_4e)  # [2, 832, 1, 14, 14]
        x = self._modules["MaxPool3d_5a_2x2"](x_4f)  # [2, 832, 1, 7, 7]
        x, _, _, _, _ = self._modules["Mixed_5b"](x)  # [2, 832, 1, 7, 7]
        x, _, _, _, _ = self._modules["Mixed_5c"](x)  # [2, 1024, 1, 7, 7]
